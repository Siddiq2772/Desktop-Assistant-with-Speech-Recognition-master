...
def handle_client(connection, address):
    try:
        while True:
            # Here you would typically handle incoming messages from the client
            # For this example, we're just keeping the connection open
            time.sleep(1)
...
def printBubble(message):
    ...
    if client_socket:
        ...
        print(f"Client not connected. Message: {message}")
...
def wiki(query):
    ...
    try: 
        ...
        printBubble("According to wikipedia")
        speak("According to wikipedia")
        printBubble(f"{result} for more information go to wikipedia.com")
        speak(f"{result} for more information go to wikipedia.com")
        
    except Exception as e:
        printBubble("Something went wrong ",e)
        speak("Something went wrong")
    

def google_search(query):
    try:
        printBubble(f"{query} Searching from google")
...
def ytvideo(video_name):
    try:
        printBubble(f"{video_name} is going to play on YouTube")
...
def temperature(city):
    ...
    if "error" not in weather_data:
        ...
        printBubble(f"The temperature in {city} is {temp_celsius}°C with {condition}.")
        speak(f"The temperature in {city} is {temp_celsius}°C with {condition}.")
    else:
        printBubble(f"Please enter valid city name")
...
def incomplete_command(complete_command):
    printBubble(f"The command you provide is incomplete command, the complete {complete_command}")
    speak(f"The command you provide is incomplete command, the complete {complete_command}")

def open_apps(app_name):
    
    try:
        printBubble(f"Openning {app_name}")
...
def restart():
    printBubble("Are you sure you want to restart your PC? (yes/no)")
    ...
    try:
        ...
        if battery is not None:
            ...
            if plugged:
                ...
                status="is Not"
            printBubble(f"Your current battery percentage is {percentage}% and currently charger {status} Plugged In")
            speak(f"Your current battery percentage is {percentage}% and currently charger {status} Plugged In")
        else:
            printBubble("Battery not found")
...
def help_function():
    ...
    printBubble(help_text)
    
          
  

def sleep():
    printBubble("Are you sure you want to Sleep your PC? (yes/no)")
...
def minimize():
    ...
    try:
        ...
        if window:
            ...
            printBubble("Current window can't recognize")
    except Exception as e:
        printBubble("Something went wrong",e)
        speak("Something went wrong")
...
def maximize():
    ...
    try:
        window = gw.getActiveWindow()
        if window:
            printBubble("Current Window is Maximizing")
...
def closewindow():
    ...
    try:
        window = gw.getActiveWindow()
        if window:
            printBubble("Current Window is Closing")
            speak("Current Window is Closing")
            window.close()
        else:
            printBubble("Current can't recognize")
...
def sleep():
    ...
    try:
        if "yes" in user:
            printBubble("Your pc is go to sleep mode")
            speak("Your pc is go to sleep mode")
            os.system("rundll32.exe powrprof.dll,SetSuspendState 0,1,0")
        else:
            printBubble("Sleep canceled")
        ...
        printBubble("Sleep canceled ",e)
        speak("Sleep canceled")

def open_website(web_name):
    try:
        webbrowser.open(f"http://{web_name}")
        printBubble(f"Opening {web_name} in your browser...")
        speak(f"Opening {web_name} in your browser...")
    except Exception as e:
        printBubble(f"Failed to open {web_name}. Error: {e}")
...
def close_apps(app_name):
    try:
        ...
        if "not running" in result:
            printBubble("Sorry I can't close the app due to security concern and permission issues, If the app you want to close is your current app then try again and say close the current window")
...
def exit_fucntion():
    ...
    if 5 <= now < 12:
        printBubble ("Goodbye! Have a great day ahead!")
        speak("Goodbye! Have a great day ahead!")
        exit()
    elif 12 <= now< 17:
        printBubble("Goodbye! Have a wonderful afternoon!")
        speak("Goodbye! Have a wonderful afternoon!")
        exit()
    elif 17 <= now < 21:
        printBubble("Goodbye! Have a pleasant evening!")
        speak("Goodbye! Have a pleasant evening!")
        exit()
    else:
        printBubble("Goodbye! Have a restful night!")
...
def current_date():
    ...
    printBubble(f"Today's date is {date}")
...
def microphone():
    os.system("start cmd /k python maingui.py")
    wish()
